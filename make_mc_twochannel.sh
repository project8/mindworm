...
do
	...
	s1=`python histo_count.py $file 4`
	s2=`python histo_count.py $file 4.5`
	s3=`python histo_count.py $file 5`
	s4=`python histo_count.py $file 5.5`
...
set title "Normalized Counts Above Cut For Simulated Signals\n Single Channel, Convolved over 20us \n  1Hz, 100K noise temp, 20s of data"
...
plot "$output" using (\$1*1e15):((\$2-$sbase1)/sqrt($sbase1)) with lines title "Counts 4+ Sigma",\
"$output" using (\$1*1e15):((\$3-$sbase2)/sqrt($sbase2)) with lines title "Counts 4.5+ Sigma",\
"$output" using (\$1*1e15):((\$4-$sbase3)/sqrt($sbase3)) with lines title "Counts 5+ Sigma",\
"$output" using (\$1*1e15):((\$5-$sbase4)/sqrt($sbase4)) with lines title "Counts 5.5+ Sigma"
GNUPLOTHERE
